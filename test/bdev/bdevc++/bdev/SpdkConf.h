...
class SpdkConf {
  public:
    SpdkConf(const IoOptions &_ioOptions);
    SpdkConf(IoDevType devType, std::string name, size_t raid0StripeSize);
    ~SpdkConf() = default;

    SpdkConf &operator=(const SpdkConf &_r);

    struct PciAddr parsePciAddr(const std::string &nvmeAddr);
    void copyDevs(const std::vector<IoDevDescriptor> &_ioDevs);

    const std::string &getBdevNvmeName() const;
    const std::string &getBdevNvmeAddr() const;
    struct PciAddr getBdevSpdkPciAddr() const;

    IoDevType getIoDevType() const { return _devType; }
    void setIoDevType(IoDevType devType) { _devType = devType; }
