...
const std::string SPDK_APP_ENV_NAME = "BdevCpp";
...
class SpdkCore {
  public:
    ...
    SpdkDevice *spBdev;
    IoOptions ioOptions;
    Poller<IoRqst> *poller;

    const static char *spdkHugepageDirname;

    void signalReady();
    bool waitReady();
    void setPoller(Poller<IoRqst> *pol) { poller = pol; }
  ...
  private:
    ...
    inline bool isNvmeInOptions() {
        return ioOptions._devs.size() ? true : false;
    }
    inline std::string getName() { return ioOptions.name; }
